...
Each fixture must identify its source repository and exact revision. Reusable dependencies should resolve through canonical `embedded-alerts/eal-*` Zed coordinates; retained Git submodules must have an explicit composition role and must not duplicate a Zed dependency in the same workspace.
